...
| 让 AI 读取工作区并完成多步骤任务 | [Agent智能体](advanced-basic/agent.md) | 整理文件、修改代码、生成报告 |
| 让 AI 连接外部服务 | [MCP](advanced-basic/mcp/README.md) | 查询数据库、调用 GitHub 或 Notion |
| 让 Agent智能体定时运行或进入群聊 | [定时任务](advanced-basic/scheduled-tasks.md) / [频道](advanced-basic/agent-channels.md) | 每天生成简报并发送到飞书 |
...
完成第一次对话后，再根据需要学习知识库或 Agent智能体。没有自动化需求时，不需要提前配置 API 网关、MCP、频道或定时任务。
...
| **知识库** | 从你导入的资料中检索相关内容 | 否，为对话或 Agent智能体提供资料 |
| **Agent智能体** | 读取工作区、调用工具并完成多步骤任务 | 是，按权限执行 |
...
4. [Agent智能体](advanced-basic/agent.md)
