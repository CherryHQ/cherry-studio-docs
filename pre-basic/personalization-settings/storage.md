...
{% hint style="danger" %}
修改数据目录是高风险操作。迁移前请先完成一次可恢复的备份，并确认备份包含所需文件。不要在迁移过程中退出应用、关机或断开目标磁盘。
{% endhint %}
...
1. 在**设置 > 数据设置**创建本地备份，或使用 [WebDAV](../data-settings/webdav.md) / [S3 兼容存储](../data-settings/s3-compatible.md)备份。
2. 若需要保留上传文件，确认没有启用“备份时跳过文件数据”。
3. 关闭正在运行的生成任务、知识库处理、文件导入和同步任务。
4. 确认目标磁盘空间充足、连接稳定且当前用户具有写权限。
5. 建议新建一个**空文件夹**作为目标。
6. 记录当前路径，并保留原目录，直到完成迁移后验证。
