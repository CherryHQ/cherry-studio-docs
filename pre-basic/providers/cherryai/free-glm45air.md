...
为了让每一位开发者和用户都能轻松体验前沿大模型的能力，**智谱向免费为 Cherry Studio 的用户开放了 GLM-4.5-Air 模型**。作为专为 Agent 应用打造的高效基础模型，GLM-4.5-Air 在性能与成本之间实现了出色平衡，是构建智能应用的理想选择。
...
2. **专项领域优化**：在代码生成、逻辑推理、Agent 交互等关键任务上进行强化训练；
...
GLM-4.5-Air 针对 Agent 应用场景进行了深度适配，具备以下实用能力：
...
该模型可灵活集成至 **Claude Code、Roo Code** 等代码 Agent 框架，也可作为任意自定义 Agent 的核心引擎使用。
...
现在就免费体验 **GLM-4.5-Air**，开启你的 Agent 开发之旅！\
