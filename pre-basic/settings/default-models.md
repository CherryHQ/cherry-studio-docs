---
description: 为 Cherry Studio V2 的默认对话、快速任务和翻译功能选择模型。
icon: robot
---

# 默认模型设置

Cherry Studio 的不同功能会在没有单独指定模型时使用默认模型。V2 当前提供三个全局角色：**默认助手模型**、**快速模型**和**翻译模型**。
...
{% hint style="info" %}
默认模型选择器只显示已启用服务商中的已启用模型，并排除嵌入、重排序和纯图像生成模型。找不到目标模型时，应先回到模型服务检查启用状态和能力标记。
{% endhint %}
...
页面包含三个模型选择器，每一项右侧还可能提供该角色的附加设置。
...
这些参数影响使用默认助手配置的场景，不会自动覆盖所有已有助手的独立设置。

{% hint style="warning" %}
同时调整 Temperature 和 Top P 可能让输出更难预测。没有明确需求时，优先保留默认值；最大输出 Token 也不应超过服务端模型的实际限制。
{% endhint %}
...
三个角色使用同一个稳定的通用对话模型。优点是配置少；缺点是后台任务和翻译也会使用同样的成本与速度。
...
如果默认模型无法工作，请通过[反馈与建议](../../question-contact/suggestions.md)中列出的官方渠道提交反馈。建议说明 Cherry Studio 版本、三个模型角色的选择、服务商、模型 ID 和脱敏后的错误提示。
