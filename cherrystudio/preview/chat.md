...
v1.9.x 的对话框工具栏从左到右默认包含以下按钮（具体顺序可由你长按拖拽自行调整）：

* **新话题**：在当前助手内创建一个新话题
* **上传图片或文档**：图片需要模型支持视觉能力；文档会被自动解析为文字作为上下文
* **网络搜索**：开启后将网页搜索结果作为上下文返回给模型，需在 [联网模式](../../pre-basic/websearch/) 中预先配置
* **网页上下文**：粘贴 URL 后，模型将以该网页内容为参考回答（部分模型原生支持，无需联网模式）
* **知识库**：将一个已建好的知识库作为上下文，详见 [知识库教程](../../knowledge-base/knowledge-base.md)
* **MCP 服务器**：启用 MCP 工具进行调用，详见 [MCP 使用教程](../../advanced-basic/mcp/)
* **生成图片**：仅在所选 **对话模型** 支持生图时显示。专门的生图模型请前往 [绘画](drawing.md)
* **快捷短语**：调用你在 [快捷短语](../../pre-basic/settings/quick-phrase.md) 中预设好的模板
* **思考模式**：对支持深度思考的模型，控制是否显示推理过程及思考预算
* **清空消息**：删除该话题下所有内容
* **清除上下文**：截断模型可见上下文（"清空消息"是删除消息本身，"清除上下文"只是让模型"忘掉"它们）
* **展开 / 收起**：让对话框变得更大或恢复，便于输入长文

输入框中还有两个**通过键盘触发**的能力：

* **`@`**：唤起模型选择器，可临时切换接下来的回复模型（保留上下文）
* **`/`**：唤起工具选择器，可快速插入快捷短语或调用斜杠命令

### 对话框右下角：Token 计数

输入框右下角显示 **预估 Token 数**，包含四个数值：`当前上下文数` / `最大上下文数`（∞ 表示无限）/ `当前输入框内消息字数` / `预估 Token 数`。
