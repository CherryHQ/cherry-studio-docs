...
这是你最常用的页面 —— 跟 AI 聊天的地方。但 Cherry Studio 把"聊天"分成了**两层结构**：助手 → 话题。第一次用难免有点懵，所以先讲清楚。

> 想顺道把 助手 / 智能体 / 技能 这些概念也理清？看 [5 分钟搞懂](../../advanced-basic/concepts-101.md)。

## 助手和话题：什么关系？

类比一下：

* **助手 = 一个角色**（比如"产品文档助理"、"严厉的代码 reviewer"）
* **话题 = 跟这个角色的一段对话**（比如周一聊"重构方案"、周二聊"bug 报告"）

也就是说：**一个助手底下可以开无数个话题**，所有话题共用这个助手的人设和参数（提示词、模型、温度等）。这样你不用每次都重新告诉 AI"你是谁、按什么风格回话"。

### 助手

`助手` 就是给 AI 套一身"人设衣服" —— 系统提示词 + 模型参数预设。

* **系统默认助手**：一个通用助手，没设特殊提示词，直接用就行
* **想要更专门的助手**？去 [助手广场](agents.md) 浏览预设，或者自己创建一个

### 话题

每个助手下可以开多个话题（也就是多段独立对话）。话题之间互不干扰，但都共享同一个助手的设置。

适合的用法：

* 用同一个"代码助手"分别开"项目 A 重构"、"项目 B bug"两个话题，各聊各的
* 用同一个"翻译助手"开多个话题，分别翻译不同的文章
