...
1. **左侧默认可见工具**：日常对话最常用的几个开关
2. **`⊙` 折叠按钮**：点击展开更多隐藏工具，再点收起
3. **右下角**：翻译当前输入、发送

所有工具都可以长按拖拽调整顺序，也可以在可见区与隐藏区之间互换。
...
| 🛠️ | **MCP 服务器** | 启用 [MCP](../../advanced-basic/mcp/) 工具供模型调用 |
...
| **清空消息** | **删除**该话题下所有消息（不可恢复） |
| **展开 / 收起输入框** | 让输入框变得更大或恢复，便于输入长文 |
...
| 图标 | 名称 | 作用 |
|---|---|---|
| 文A | **翻译** | 将输入框内容直接翻译为目标语言（在 `设置 → 默认模型` 中配置默认翻译模型） |
| ⬆️ | **发送** | 发送消息（默认 Enter；可在 [快捷键](../../pre-basic/settings/key-shortcut.md) 中改）|

### 仅在符合条件时显示的工具

下列工具不在默认工具栏中，**仅当所选模型 / 助手支持时才在分隔线左侧出现**：

* **生成图片**：所选对话模型支持生图时出现。专门的生图模型请去 [绘画](drawing.md)
