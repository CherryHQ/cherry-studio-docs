...
This page shows the top 100 models from the [Arena Text leaderboard](https://arena.ai/leaderboard/text), giving you a practical shortlist to test in Cherry Studio. An automated job updates the page daily. Use the Arena website for the full leaderboard, filters, and latest statistics.

> **Fetch time**: 2026-07-25 01:44:22 UTC / 2026-07-25 09:44:22 CST (Beijing time)

## How to read the leaderboard

- **Rank**: The model's position on the current Text Arena leaderboard.
- **Rank range**: The range of plausible ranks reported by Arena for the current statistics. A narrower range usually indicates a more stable relative position.
- **Model**: Arena's model name and public information link. This name may differ from the model ID required by an API provider.
- **Score**: A relative preference score derived from anonymous head-to-head votes. It is useful for comparisons within the current leaderboard, but it is not an absolute accuracy measure.
- **Votes**: The number of preference votes included in the calculation. Models with fewer votes or a `Preliminary` label can move more quickly.
- **Price**: Arena's reference input/output price in US dollars per million tokens.
- **Context**: The reference context-window size recorded by Arena.

Prices, context windows, and license information can change when providers update their services. Check the provider's official documentation and console before connecting a model.

## Leaderboard

| Rank | Rank range | Model | Score | Votes | Price (USD/million tokens) | Context |
| --- | --- | --- | --- | --- | --- | --- |
...
## Choose a model for Cherry Studio

1. Shortlist a few models based on your workload, budget, and required context length.
2. Fetch the latest model list from the corresponding provider in Cherry Studio, or enter the exact model ID required by that provider.
3. Test each candidate with the same prompts, attachments, and parameters. Compare output quality, latency, reliability, and actual cost.
4. Set separate defaults for chat, coding, translation, long documents, and image tasks instead of relying on a single overall rank.

Arena results reflect anonymous user preferences within a particular time window and set of filters. Style control, factuality adjustments, and leaderboard filters can all affect the result. Treat the leaderboard as a source of candidates, not the sole basis for purchasing or configuration decisions.

## Data source and updates

Data comes from the [official Arena Text leaderboard](https://arena.ai/leaderboard/text) and is fetched and generated daily by GitHub Actions in this repository. This page keeps only the top 100 entries for readability. Open the official leaderboard to see every model, category-specific rankings, or different filter settings.
