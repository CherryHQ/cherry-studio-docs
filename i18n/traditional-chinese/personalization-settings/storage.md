...
{% hint style="danger" %}
修改資料目錄是高風險操作。移轉前請先完成一次可以還原的備份，並確認備份包含需要的檔案。請勿在移轉過程中退出應用程式、關機或拔除目標磁碟。
{% endhint %}
...
1. 在**設定 > 資料設定**中建立本機備份，或使用 [WebDAV](../data-settings/WebDAV.md) / [S3 相容儲存空間](../data-settings/s3-compatible.md)進行備份。
2. 如果需要保留上傳的檔案，請確認未啟用「備份時略過檔案資料」。
3. 關閉正在執行的產生工作、知識庫處理、檔案匯入和同步工作。
4. 確認目標磁碟有足夠空間、連線穩定，而且目前使用者具有寫入權限。
5. 建議建立一個**空白資料夾**作為目標。
6. 記錄目前的路徑，並保留原始目錄，直到完成移轉後的驗證。
