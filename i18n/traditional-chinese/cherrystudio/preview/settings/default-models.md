---
description: 為 Cherry Studio V2 的預設對話、快速任務和翻譯功能選擇模型。
icon: robot
---

# 預設模型設定

Cherry Studio 的不同功能會在未個別指定模型時使用預設模型。V2 目前提供三個全域角色：**預設助理模型**、**快速模型**和**翻譯模型**。
...
{% hint style="info" %}
預設模型選擇器只顯示已啟用服務商中的已啟用模型，並排除嵌入、重新排序和純圖像產生模型。找不到目標模型時，應先回到模型服務檢查啟用狀態和能力標記。
{% endhint %}
...
頁面包含三個模型選擇器，每一項右側還可能提供該角色的附加設定。
...
這些參數會影響使用預設助理設定的情境，不會自動覆寫所有既有助理的獨立設定。

{% hint style="warning" %}
同時調整 Temperature 和 Top P 可能讓輸出更難預測。沒有明確需求時，優先保留預設值；最大輸出 Token 也不應超過伺服器端模型的實際限制。
{% endhint %}
...
三個角色使用同一個穩定的通用對話模型。優點是設定較少；缺點是背景任務和翻譯也會使用相同的成本與速度。
...
如果預設模型無法運作，請透過[意見回饋](../../../question-contact/suggestions.md)中列出的官方管道提交意見。建議說明 Cherry Studio 版本、三個模型角色的選擇、服務商、模型 ID 和經過遮蔽處理的錯誤提示。
