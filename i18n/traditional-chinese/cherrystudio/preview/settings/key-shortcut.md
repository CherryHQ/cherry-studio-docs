...
| 分組 | 包含的操作 |
| --- | --- |
| 全域與視窗 | 顯示或隱藏應用程式、開啟設定、切換側邊欄、介面縮放和全域搜尋 |
| 訊息互動 | 清空訊息、清除上下文、搜尋訊息、複製或編輯訊息、選擇模型 |
| 工作階段與話題 | 新增話題、重新命名話題、顯示或隱藏話題清單 |
| AI 助理工具 | 快捷助理和劃詞助理的全域快速鍵 |

功能未啟用時，對應的快速鍵不會顯示。例如，只有啟用快捷助理後，**AI 助理工具**中才會出現快捷助理；劃詞助理的兩項快速鍵也僅支援 macOS 和 Windows。

頁面上方的搜尋框只會搜尋目前分組，可以依操作名稱或目前顯示的組合鍵篩選。
...
**全部啟用**和**全部停用**只會影響目前分組中、目前篩選結果內且已經綁定按鍵的項目，不會修改未綁定的項目。
...
| 操作 | 預設綁定 | 預設狀態 | 說明 |
| --- | --- | --- | --- |
| 清空訊息 | `Cmd/Ctrl + L` | 啟用 | 確認後刪除目前話題中的所有訊息 |
| 在目前的對話中搜尋訊息 | `Cmd/Ctrl + F` | 啟用 | 在目前話題內尋找訊息 |
| 清除上下文 | `Cmd/Ctrl + K` | 啟用 | 在目前話題中切換上下文分隔點，不會刪除歷史訊息 |
...
{% hint style="warning" %}
**清空訊息**與**清除上下文**不是同一項操作。清空訊息會在確認後刪除目前話題的所有訊息；清除上下文只會讓後續對話從新的上下文區段開始，原有訊息仍會保留在頁面中。再次觸發清除上下文，可以移除末尾尚未使用的分隔點。
{% endhint %}
...
| 開關劃詞助理 | 未綁定 | 停用 | 請先啟用劃詞助理；僅限 macOS 和 Windows |
| 劃詞助理：取詞 | 未綁定 | 停用 | 請先啟用劃詞助理；僅限 macOS 和 Windows |
...
劃詞助理快速鍵只有在功能已經啟用時才會顯示，而且目前僅支援 macOS 和 Windows。Linux 不會顯示這兩項快速鍵。

### 為什麼「全部啟用」沒有開啟某些項目？

批次操作只會處理目前分組和目前搜尋結果中已經綁定按鍵的項目。對於顯示「按下快速鍵」的未綁定項目，需要先逐項設定組合鍵。
