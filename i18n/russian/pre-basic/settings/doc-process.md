...
Обработка документов используется для преобразования изображений в чистый текст или преобразования таких документов, как PDF, в Markdown для последующего поиска, парсинга и индексации в базе знаний.
...
Для каждой возможности выбирается отдельный обработчик по умолчанию. Если одна служба поддерживает обе возможности, она появится дважды в списке, а конфигурация и состояние по умолчанию управляются отдельно.

## Настройка обработки документов

Перейдите в **Настройки > Анализ документов**.

В левой части будут перечислены доступные возможности для текущего устройства в следующем порядке:

1. Изображение в текст: System OCR, Tesseract OCR, PaddleOCR, Mistral, Intel OV OCR.
2. Документ в Markdown: Mistral, MinerU, Doc2x, Open MinerU, PaddleOCR.

Обработчики будут фильтроваться в зависимости от платформы и среды выполнения. Если список отличается от данного текста, следует руководствоваться фактическим отображением в приложении.
...
На странице PaddleOCR можно выбрать:
...
Встроенное значение по умолчанию — `PaddleOCR-VL-1.5`. Выбранная модель должна поддерживаться целевым сервисом PaddleOCR; доступные модели для собственного развёртывания могут отличаться от облачных.
...
На текущей странице настроек анализа документов нет отдельной кнопки «Проверка подключения». Самый надёжный способ проверки:
