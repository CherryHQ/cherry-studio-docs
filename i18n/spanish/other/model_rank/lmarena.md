...
Este es un leaderboard basado en datos de Chatbot Arena (lmarena.ai), generado automáticamente mediante un proceso automatizado.

> **Última actualización de datos**: 2025-09-10 11:40:58 UTC / 2025-09-10 19:40:58 CST (Hora de Pekín)
...
|       12 |               7 | [Qwen3-235B-A22B-Thinking-2507](https://huggingface.co/Qwen/Qwen3-235B-A22B-Thiking-2507)                                      | 1413 | +9/-9   | 3,715   | Alibaba                | Apache 2.0              | nan      |
...
|       33 |              极4 | [Mistral Medium 3](https://mistral.ai/news/mistral-medium-3)                                                                    | 1370 | +5/-5   | 28,010  | Mistral                | Proprietary             | nan      |
...
|       36 |              27 | [Claude Opus 4 (20250514)](https://www.anthropic.com/news/claude-4)                                                            极 1366 | +5/-5   | 26,141  | Anthropic             | Proprietary             | nan      |
...
|       49 |              44 | [Qwen3-32B](https://qwenlm.github.io/blog/qwen3/)                                                                               | 1341 | +9/-9   | 4,074巨  | Alibaba                | Apache 2.0              | nan      |
...
<TRUNCATED - FULL TABLE AVAILABLE IN ORIGINAL DOCUMENT>

## Explicación

- **Posición (UB)**: Clasificación calculada mediante el modelo de Bradley-Terry. Refleja el rendimiento integral del modelo en el arena y proporciona una estimación de **límite superior** de su puntuación Elo, ayudando a comprender la competitividad potencial del modelo.
- **Posición (StyleCtrl)**: Clasificación después de aplicar control de estilo conversacional. Pretende reducir el sesgo de preferencias causado por factores de estilo (por ejemplo, verbosidad, concisión) y evaluar las capacidades principales de manera más pura.
- **Nombre del modelo**: Nombre del modelo de lenguaje grande (LLM). Esta columna incluye enlaces relacionados; consulte haciendo clic.
- **Puntuación**: Calificación Elo obtenida del modelo a través de votaciones de usuarios en el arena. El sistema de clasificación relativa Elo indica que un valor más alto refleja un mejor desempeño. Esta puntuación cambia dinámicamente y refleja la fortaleza relativa del modelo en el entorno actual.
- **Intervalo de confianza**: Intervalo de confianza del 95% para la calificación Elo (ejemplo: `+6/-6`). Un intervalo menor indica una mayor estabilidad y fiabilidad de la calificación; un intervalo grande sugiere escasez de datos o alta variabilidad en el rendimiento. Proporciona una evaluación cuantitativa de la precisión de la puntuación.
- **Votos**: Número total de votos recibidos por el modelo en el arena. Generalmente, más votos implican mayor confiabilidad estadística de la puntuación.
- **Proveedor**: Organización o empresa que provee el modelo.
- **Licencia**: Tipo de licencia del modelo (por ejemplo, Propietario / Proprietary, Apache 2.0, MIT).
- **Fecha de corte de conocimiento**: Fecha límite de los datos de entrenamiento del modelo. **N/A** indica información ausente o desconocida.

## Fuente de datos y frecuencia de actualización

Los datos de este leaderboard son generados automáticamente por [fboulnois/llm-leaderboard-csv](https://github.com/fboulnois/llm-leaderboard-csv), proyecto que obtiene y procesa datos de [lmarena.ai](https://lmarena.ai/). Este leaderboard se actualiza diariamente mediante GitHub Actions.

## Descargo de responsabilidad

Este informe es solo para referencia. Los datos del leaderboard son dinámicos y están basados en votos de preferencia de usuarios en Chatbot Arena durante periodos específicos. La integridad y precisión dependen de fuentes de datos ascendentes y del procesamiento en `fboulnois/llm-leaderboard-csv`. Los modelos pueden tener diferentes licencias: consulte siempre información oficial de los proveedores.