...
ページ左側は用途別に 4 つのグループに分かれています。

| グループ | 含まれる操作 |
| --- | --- |
| グローバルとウィンドウ | アプリの表示 / 非表示、設定を開く、サイドバーの切り替え、インターフェースのズーム、グローバル検索 |
| メッセージ操作 | メッセージをすべて削除、コンテキストをクリア、メッセージの検索、メッセージのコピーまたは編集、モデルの選択 |
| 会話とトピック | トピックの新規作成、トピックの名前変更、トピックリストの表示 / 非表示 |
| AI アシスタントツール | クイックアシスタントおよびテキスト選択ツールのグローバルショートカット |

機能が有効になっていない場合、対応するショートカットは表示されません。たとえば、クイックアシスタントを有効にした場合にだけ、**AI アシスタントツール**にクイックアシスタントが表示されます。テキスト選択ツールの 2 つのショートカットは macOS と Windows でのみサポートされています。

ページ上部の検索ボックスは現在のグループ内だけを対象とし、操作名または現在表示されているショートカットで絞り込めます。
...
**すべて有効化**と**すべて無効化**は、現在のグループのうち、現在の検索結果に含まれ、すでにショートカットが設定されている項目だけに適用されます。未設定の項目は変更されません。
...
| サイドバーの切り替え | `Cmd/Ctrl + [` | 有効 | アシスタントのサイドバーを切り替えるか、フォーカスします。 |
...
| 操作 | デフォルトショートカット | デフォルト状態 | 説明 |
| --- | --- | --- | --- |
| メッセージをすべて削除 | `Cmd/Ctrl + L` | 有効 | 確認後、現在のトピックのすべてのメッセージを削除します。 |
| 現在の会話内でメッセージを検索 | `Cmd/Ctrl + F` | 有効 | 現在のトピック内でメッセージを検索します。 |
| コンテキストをクリア | `Cmd/Ctrl + K` | 有効 | 現在のトピックでコンテキスト区切り点を切り替え、履歴メッセージは削除しません。 |
...
{% hint style="warning" %}
**メッセージをすべて削除**と**コンテキストをクリア**は同じ操作ではありません。メッセージをすべて削除は、確認後に現在のトピックのすべてのメッセージを削除します。一方、コンテキストをクリアは、後続の対話が新しいコンテキストセグメントから開始されるようにするだけであり、既存のメッセージはページ上に保持されます。再度コンテキストをクリアすると、末尾に残っている未使用の区切り点を削除できます。
{% endhint %}
...
| トピック表示の切り替え | `Cmd/Ctrl + ]` | 有効 | トピックリストを切り替えるかフォーカスします。 |
...
| テキスト選択ツールのオン / オフ | 未設定 | 無効 | 先にテキスト選択ツールを有効にします。macOS と Windows でのみ使用できます。 |
| テキスト選択ツール：テキストを取得 | 未設定 | 無効 | 先にテキスト選択ツールを有効にします。macOS と Windows でのみ使用できます。 |
...
テキスト選択ツールのショートカットは、機能が有効な場合だけ表示されます。現在は macOS と Windows のみ対応しており、Linux ではこの 2 項目は表示されません。

### 「すべて有効化」を押しても一部の項目が有効にならないのはなぜですか？

一括操作は、現在のグループと検索結果のうち、すでにショートカットが設定されている項目だけに適用されます。「ショートカットを入力」と表示される未設定の項目は、先に 1 つずつキーの組み合わせを設定してください。
