...
嵌入模型会把文本转换为向量，供知识库检索相似内容。它和生成回答的模型职责不同：嵌入模型负责找到相关资料，生成模型负责根据这些资料组织最终回答。因此，嵌入模型会直接影响“能不能找到正确资料”。
...
如果没有特殊要求，优先选择 Provider 当前提供的通用多语言嵌入模型。常见可选模型例如 **BGE M3**、`text-embedding-3-small`；这些名称仅作示例，实际可用性取决于服务商、账号及当前模型列表。不要只根据名称中的参数量判断效果。
...
5. 进入目标知识库，打开 **知识库设置 → 嵌入模型**，直接从下拉框选择它。
