...
如果你想要"**每天早上 9 点自动给我做一份昨天的新闻简报**"、"**每周一汇总团队工作量**"、"**每小时同步一次数据看板**"，每次手动点一遍太累 —— 定时任务就是给你的 [Cherry Agent](agent.md) 一份**"值班表"**，让它按时间表自动出工。

* **是什么**：让 Cherry Agent 按你设的时间表自动跑
* **能干什么**：任何你已经能让 Agent 做的事，都能"定时做"
* **怎么知道做完了**：每次运行都有日志记录可查

> 不清楚 Agent 是什么？先看 [5 分钟搞懂](concepts-101.md)。
