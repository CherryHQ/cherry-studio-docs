...
定时任务为 [Cherry Agent](agent.md) 提供按计划自动运行的能力，适用于"每日早上 9 点生成新闻简报"、"每周一汇总团队工作量"、"每小时同步数据看板"等周期性需求。
...
{% hint style="info" %}
**前提**：Agent 在编辑面板中已开启 **自主模式**（基础设置 Tab），并将 **权限模式** 设为 **全自动模式**。详见 [Cherry Agent](agent.md)。
...
未满足上述条件的 Agent **不会**出现在新建任务的下拉中。两类设置的详细说明见 [Cherry Agent](agent.md#di-5-bu-tiao-zheng-zhi-neng-ti-de-ti-shi-ci-gong-ju-yu-ji-neng)。
