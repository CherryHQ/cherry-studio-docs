...
图中：① 四步创建流程；② 主模型；③ Agent 的用途说明；④ 下一步或新建按钮。
...
* 【基础设置】：主模型、Plan 模型、Small 模型、权限模式和心跳检测；
* 【提示词】：角色说明、处理规则和行为边界；
* 【工具】→【内置工具】：文件、搜索、图片、通知、定时任务、记忆、子智能体和工作流等；
* 【工具】→【知识库】：限制 Agent 可以访问的知识库；
* 【工具】→【MCP】：绑定已经连接的 MCP 服务器；
* 【工具】→【技能】：选择已安装技能；
* 【高级设置】：为确实需要的工具设置环境变量。
...
创建向导只保留常用步骤。创建完成后打开 Agent 编辑窗口，可在【基础设置】和【工具】中配置权限、内置工具、MCP 及更多模型角色。
