...
图中：① 【开发者模式】；② 重启提示；③ 【调用链】；④ 节点输入、输出、耗时和状态。
...
先在 MCP 设置页确认服务器正常，再让 Agent 只调用一个工具。调用链显示请求已到达服务器但输出为空，说明问题更可能在服务端数据或参数，而不是 Agent 没有绑定；若根本没有 MCP 节点，则回到 Agent【工具】→【MCP】检查绑定。
