...
图中：① 已安装技能；② 在线搜索；③ 从 ZIP 或文件夹安装；④ 查看技能内容与卸载。
...
打开【工作】→ Agent 菜单→编辑→【工具】→【技能】，启用所需技能。技能切换会随 Agent 配置自动保存，并从下一条消息生效。
{% endstep %}
...
安装只把技能加入全局能力库。还需要在 Agent 编辑窗口的【工具】→【技能】中启用，或让 Agent 帮你完成绑定。随后新发一条消息测试。
