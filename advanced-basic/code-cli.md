...
【Code CLI】用于安装、配置和启动常见编程命令行工具。Cherry Studio 会识别自己的托管安装，也会检测系统登录环境中已经可用的 CLI；系统工具仍由原来的包管理器维护。
...
页面当前包含 Claude Code、OpenAI Codex、Gemini CLI、OpenCode、Qwen Code、Kimi CLI、Qoder CLI 和 GitHub Copilot CLI 等工具。实际可见项会随产品更新变化，以页面列表为准。

图中：① 工具列表；② 安装来源与版本状态；③ 服务商和模型；④ 工作目录与终端；⑤ 【启动】。
...
| ---------------- | ------------------ | --------------------- |
| Cherry Studio 托管 | 安装、更新和卸载对应托管副本     | 在【Code CLI】或【环境依赖】中管理 |
