...
* **技能（Skill）** = 加装在智能体身上的"专业能力包"，例如"会做 PPT"、"会撰写专利申请"，开箱即用
...
* 智能体 = 可读取工作区、调用工具并**执行多步骤任务**的同事
{% endhint %}

### 3. 技能（Skill）—— 为 Agent 提供可复用的工作方法

智能体本身聪明但属于"通才"。若你经常处理某类专项任务（如小红书图文、PPT、财务分析），可以启用包含操作说明、模板和资源的**技能**，让 Agent 按更稳定的流程工作。
...
> **Provider** 提供模型能力 → **助手 / 智能体** 是有特定角色的同事 → **智能体**可通过技能习得专业本领 → 助手和智能体可通过 **MCP** 连接外部工具 → 智能体可通过**频道**派驻至 IM 平台，或通过**定时任务**按计划自动运行
...
4. **第四步**：实现自动化 → 开启 [API 网关](api-server.md) → 配置 [智能体](agent.md)
