...
# 核心概念：助手 / Agent / 知识库 / 技能 / MCP

Cherry Studio 将模型、资料和工具组织为不同能力。本页说明每项能力解决什么问题、什么时候使用，以及它们如何组合；具体配置步骤见对应教程。

## 能力对照

| 能力 | 作用 | 适合的场景 |
|---|---|---|
| **模型服务（Provider）** | 提供生成回答所需的模型 | 决定可用模型、费用与数据政策 |
| **助手（Assistant）** | 保存对话规则和回复风格 | 固定角色的日常问答、写作、翻译或编程 |
| **Agent** | 读取工作区、调用工具并执行多步骤任务 | 整理文件、分析项目、修改代码或生成报告 |
| **知识库** | 检索导入的文档并提供相关内容 | 基于产品手册、课程资料、合同或笔记回答 |
| **技能（Skill）** | 为 Agent 提供可复用的流程、模板和资料 | 固定流程的专项任务 |
| **MCP** | 让助手或 Agent 使用外部工具和数据源 | 查询数据库、操作 Notion、GitHub 或其他服务 |
| **频道** | 将 Agent 接入 IM 平台 | 在飞书、Telegram、QQ、微信、Discord 或 Slack 中响应 |
| **定时任务** | 按计划触发 Agent | 日报、周报、数据同步等周期性任务 |
