...
* 频道触发的任务无人在线确认，建议检查该 Agent 的工具开关和可访问目录，避免授予超出任务需要的权限。详见 [智能体](agent.md)。
{% endhint %}
...
1. 已创建一个 [智能体](agent.md)。工作区不是必填项；需要 Agent 访问本地文件时再为频道选择工作区
...
|---|---|
| **名称** | 频道的展示名，便于在多频道时辨识 |
| **绑定 Agent** | 选择一个已创建的 Cherry Agent。未绑定 Agent 时频道不会连接，也不会回复消息 |
