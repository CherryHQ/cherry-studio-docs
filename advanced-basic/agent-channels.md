...
当你在 Cherry Studio 中配置好一个 Agent（如"研报机器人"、"客服助手"），如果希望它**驻留在 IM 群中**为多人服务，可以使用「频道」功能。

**频道**将 [Agent](agent.md) **接入 IM 平台**作为机器人对外服务。
...
* **公司内部知识机器人**：飞书群成员 @ 机器人提问，Agent 通过已启用的工具或 MCP 查询内部资料
...
频道功能涉及 "创建第三方平台机器人 → 拿到凭据 → 填进 Cherry Studio" 几个步骤，对非技术用户来说门槛偏高。**如果不熟悉这些操作，可以把任务直接交给一个 Agent**。

在 Agent 对话窗口中描述你的目标即可，例如：

> "请帮我设置一个频道：每天早上 10 点把虎嗅、36 氪、机器之心三家媒体的 AI 相关新闻汇总成 5 条要点，发到我的飞书。"

Agent 会判断需要做哪些事，向你索取必要的凭据（飞书 App ID 等），然后帮你完成频道创建与 [定时任务](scheduled-tasks.md) 配置。

{% hint style="warning" %}
**V2 权限说明**：
* V2 已移除「自主模式」开关，所有 Agent 默认具备原自主模式提供的任务管理、记忆与工作区身份能力。
* 频道可以绑定任意 Agent，不再要求开启自主模式或选择全自动权限。
* 频道触发的任务无人在线确认，建议检查该 Agent 的工具开关和可访问目录，避免授予超出任务需要的权限。详见 [Agent](agent.md)。
{% endhint %}
...
1. 已创建一个 [Agent](agent.md)。工作区不是必填项；需要 Agent 访问本地文件时再为频道选择工作区
2. 已启用 [API 网关](api-server.md)
...
1. 在左侧列表中点击 **飞书**
2. 在右侧空态点击 **+ 添加**。系统会立即创建一个频道，并打开编辑窗口：
...
|---|---|
| **名称** | 频道的展示名，便于在多频道时辨识 |
| **绑定 Agent** | 选择一个已创建的 Agent。未绑定 Agent 时频道不会连接，也不会回复消息 |
| **应用 ID / 应用密钥** | 飞书开放平台 → 自建应用 → 凭证与基础信息 |
| **加密密钥 / 验证令牌** | 飞书开放平台 → 事件订阅，可选 |
| **域名** | `飞书（中国）` / `Lark（国际版）`，二选一 |
| **允许的聊天 ID** | 留空表示不限制；填入则只在指定群/单聊中响应 |
| **频道权限模式** | `继承 Agent 设置` / 自定义。建议保持继承 |

字段在失去焦点时会自动保存，没有单独的“保存”按钮。Agent 与平台凭据配置完整后，已启用的频道会自动开始订阅飞书消息事件。
...
* 确认 [API 网关](api-server.md) 处于 **运行中** 状态
